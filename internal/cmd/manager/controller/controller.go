...
func RunController(
	...
	// Use intelligent certificate selection with improved error handling
	...
	if err != nil {
		if conf.WebhookCertDir != "" {
			// Custom cert directory specified but selection failed - this is an error
			setupLog.Error(err, "failed to select webhook certificate names in custom directory")
			return err
		} else {
			// No custom cert directory - fallback to standard names
			setupLog.Info("Using standard certificate names", "reason", err.Error())
			certName = "tls.crt"
			keyName = "tls.key"
		}
	}
